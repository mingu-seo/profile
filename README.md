# 개발자겸 강사 서민구

> "누구나 쉽게 '코딩'을 시작하고 이를 도구로 사용할 수 있도록 돕고 영감을 주는 사람"

## About Me

**15년간 개발자**로, **7년간 IT 강사**로 활동하며,
비전공자부터 실무 개발자까지 누구나 효과적으로 코딩을 배우고 활용할 수 있도록 돕고 있습니다.

### 강의 분야

- 웹, 백엔드 개발
- 데이터 분석
- Java, Python 프로그래밍
- DataBase, SQL
- AI, Machine Learning
- 시스템

---

## Career

### 개발 경력 (15년)

- 다양한 기업에서 웹 개발, 백엔드 시스템 구축, 소프트웨어 아키텍처 설계 경험
- Java, Spring, Python 기반 프로젝트 다수 수행
- 기업 내부 시스템 개발 및 운영 경험
- 개발팀장 및 PM 업무 경험

### 강의 경력 (7년)

- 현) **신한DS SW 아카데미 강사**
- Java 풀스택 개발 강의
- AI / Machine Learning
- 기업 및 기관 재직자 대상 프로그래밍, DB 강의
- 기업 대상 영상 강의
- 한양대학교 ERICA 겸임교수

---

## Education

- 소프트웨어공학 석사 ([학위논문](https://www.riss.kr/search/detail/DetailView.do?p_mat_type=be54d9b8bc7cdb09&control_no=0b956a1ed7f0f0b3ffe0bdc3ef48d419))
- 디지털정보학 학사
- 통계데이터과학 학사

---

## Certifications

| 자격증 |
|--------|
| 직업훈련교사 정보기술개발 2급 |
| 직업훈련교사 정보기술 전략계획 3급 |
| 직업훈련교사 인공지능 3급 |
| 정보처리기사 |
| 정보보안기사 |
| 빅데이터분석기사 |
| 리눅스마스터 1, 2급 |
| SQLD |
| ADsP |

---

## Books

- **《실전 예제로 배우는 자바 프로그래밍》** (엠제이씨북스, 2020)

---

## Contact

[![Email](https://img.shields.io/badge/Email-codro.ceo@gmail.com-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:codro.ceo@gmail.com)
[![Instagram](https://img.shields.io/badge/Instagram-@codro.it-E4405F?style=flat-square&logo=instagram&logoColor=white)](https://www.instagram.com/codro.it/)
[![Website](https://img.shields.io/badge/Website-codroworld.com-4285F4?style=flat-square&logo=googlechrome&logoColor=white)](http://codroworld.com)

> 프로그래밍 교육 및 강의, 개발 의뢰가 필요하시다면 언제든지 연락 주세요.
